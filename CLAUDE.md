...
- **`utils/workspace/`** holds the imperative/vanilla-JS-style helpers that back the canvas, separate from React state:
  ...
  - `navigator.ts` (`NavigatorController`) — drives the mini-map (`components/editor/navigator/Navigator.tsx`): sizes the minimap world `div` off `ENV.worldDims` and a fixed `scaleVal`, and keeps it in sync via a `ResizeObserver` on the real world element. The minimap's `camera` div (meant to show the current viewport within the world) is currently just a static, unwired placeholder — it doesn't track pan/zoom yet.
...
There is currently no consistent state-management approach: components mostly use plain `useState`, plus a couple of imperative singletons (`patientLoad`, `NavigatorController`). An earlier `reducers/navigator.tsx` (a Redux-style reducer, no `redux` dependency involved) was removed for being unused/never wired into a `useReducer` call — its removal wasn't a decision that a reducer-based approach is wrong, just that this particular one was dead code. Whether/how to introduce a more structured approach (context + reducer, a lightweight store, or continuing with `useState` + singletons) is an open question to revisit once more cross-node state (e.g. the patch-cable/connection model) exists to design around.

### Audio engine

Sound is not implemented yet — `Polysynth` and `Keyboard` are currently visual mockups only, with no node-graph/patch-cable model. When implementing audio, use **Tone.js** as the layer on top of the Web Audio API (not raw `AudioContext`/`AudioNode`s directly) — e.g. `Tone.PolySynth`, `Tone.Oscillator`, `Tone.Transport`. `tone` **is already installed** (`tone@15.1.22`); `components/nodes/midi-box/MidiBox.tsx` and `components/effects/filter/filter-response.ts` already `import * as Tone from "tone"` directly, ahead of the facade below being built — new audio code should prefer going through the facade once it exists rather than adding more direct `Tone` imports.
